...
double *degree_old, *degree;	// node degree
set<int> *p_old, *p;	// maintains topology
...
int main(int argc, char * argv[])
{
	...
	if(argc==8){
		...
		transform_wlen=atoi(argv[7]);
	}
	else{
		cout << " Usage : social-networks.exe num_nodes num_dishonest num_sybil random_seed graph_file walk_length transform_wlen" << endl;
		exit(1);
	}
	...
	degree_old=new double[num_nodes+attack_nodes];	
	for(int i=0;i<num_nodes+attack_nodes;i++){
		degree[i]=0;
		degree_old[i]=0;
	}
	p= new set<int>[num_nodes+attack_nodes];
	p_old= new set<int>[num_nodes+attack_nodes];
	...
	graph_transformation(); // transform the topology based on our perturbation algorithm
...
void attacker_strategy(){
	...
	for(int i=num_nodes;i<num_nodes+attack_nodes;i++){
		for(int j=0;j<m;j++){
			...
			if(bad_iter!=bad.end()){
				p_old[i].insert(*bad_iter);
				p_old[*bad_iter].insert(i);
				degree_old[i]++;
				degree_old[*bad_iter]++;
			}
			else{
				p_old[i].insert(bad_last);
				p_old[bad_last].insert(i);
				degree_old[i]++;
				degree_old[bad_last]++;
			}
		}
...
void graph_transformation(){
	int prev_hop,cur_hop,next_hop;
	int random,count_neighbors;
	set<int>::iterator iter,iter2;
	for(int i=0; i< num_nodes+attack_nodes;i++){
		for(iter=p_old[i].begin();iter!=p_old[i].end();iter++){
			next_hop=*iter;
			int num_loop=0;
			do{
				next_hop=*iter;
				for(int j=1;j<transform_wlen;j++){
					...
					for(iter2=p_old[cur_hop].begin();iter2!=p_old[cur_hop].end();iter2++){
						sum=sum+1.0/(double)p_old[cur_hop].size();
						//cout << "Current sum: " << sum << endl;
						if(sum >= rand_value){
							break;
						}
					}
					if(iter2==p_old[cur_hop].end()){
						iter2=p_old[cur_hop].begin();
					}
					next_hop=*iter2;
				}
				num_loop++;
			}while((next_hop==i || p[i].find(next_hop)!=p[i].end()) && num_loop < 5);

			if(iter==p_old[i].begin()){
				p[i].insert(next_hop);
				degree[i]++;
				p[next_hop].insert(i); 
				degree[next_hop]++;
				...
				double num_degree= (double)p_old[i].size();
			 	double rand_value = (double) ((float)1.0*rand()/(RAND_MAX+1.0));
