...
int main(int argc, char *argv[]){
	...
	unordered_map<vertex,int> map_vertex;
	int count=0;
	for(iter=g_transform->vertices.begin();iter!=g_transform->vertices.end();iter++){
		map_vertex[*iter]=count;
		count++;
	}
	for(iter=g_transform->vertices.begin();iter!=g_transform->vertices.end();iter++){
		cout << map_vertex[*iter];
		for(iter2=g_transform->adjacency[*iter].begin();iter2!=g_transform->adjacency[*iter].end();iter2++){
			cout << " " << map_vertex[*iter2];
		}
		cout << endl;
	}
	exit(1);
