...
public:
  ...
  //       fclose(intra_deg_sybil_handler);
		...
		FILE* component_handler = fopen("../result/component-size-distribution-detail.txt", "w");
        
        int max_c = 0;
        list<unordered_map<vertex, set<vertex> > > :: iterator iter_c;
        set<vertex> sybil_connected;

        unordered_map<vertex, set<vertex> > :: iterator iter_c_comp;
        for (iter_c = components.begin(); iter_c != components.end(); iter_c++) {
            for (iter_c_comp = (*iter_c).begin(); iter_c_comp != (*iter_c).end(); iter_c_comp ++ ) {
                fprintf(component_handler, "%d %d\n", iter_c_comp->first, (*iter_c).size());
                sybil_connected.insert(iter_c_comp->first);
            }
   ...
   //          }
        ...
        for (int i = 0; i < total_nodes; i++) {
          if (sybil_set[i] == 1) {
            if (sybil_connected.find(i) == sybil_connected.end()) {
              // isolated sybil nodes
              fprintf(component_handler, "%d %d\n", i, 1);
            }
